...
int main(int argc, char **argv)
{
  ...
  //パブリッシャの作成
  ros::Publisher arm_pub = nh.advertise<trajectory_msgs::JointTrajectory>("/dynamixel_workbench/joint_trajectory",1);
  ...
  while(ros::ok()) {
    ...
    jtp0.points[0].time_from_start = ros::Duration(0.5);  //実行時間(1秒かけて移動)
